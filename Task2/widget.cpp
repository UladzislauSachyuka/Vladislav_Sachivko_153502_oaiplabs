    ...
    , ui(new Ui::Widget)
{
    ...
    //scene->setSceneRect(10,10,711,571);
...
void Widget::mousePressEvent(QMouseEvent *event)
{
    if(CanveMod)
    {
        if (event->button() == Qt::LeftButton && !RightClicked){
            if (FirstP){
                scene->addLine(PreviousPoint.x() - 360,PreviousPoint.y() - 295, event->pos().x() - 360, event->pos().y() - 295, QPen(Qt::black,5,Qt::SolidLine,Qt::RoundCap));
            }
            else{
                scene->addEllipse(event->pos().x() - 848,event->pos().y() - 492, 4, 4, QPen(Qt::NoPen), QBrush(Qt::black));
        ...
        if (event->button() == Qt::RightButton){
            RightClicked = true;
            scene->addLine(PreviousPoint.x() - 360,PreviousPoint.y() - 295, FirstPoint.rx() - 360, FirstPoint.ry() - 295, QPen(Qt::black,5,Qt::SolidLine,Qt::RoundCap));
        }
    }

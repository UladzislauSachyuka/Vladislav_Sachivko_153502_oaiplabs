...
public:
    struct Node
    {
        ...
        Node() {}
        Node(T1 key, T2 val) : pair(key, val), height(1), left(nullptr), right(nullptr) {}
    };
    ...
    {
        return insert(key, T2()).pair.first;
    }
...
private:
    Node* insert(Node* head, T1 k, T2 val)
    {
        if (!head)
        {
            ++sz;
        ...
        if (k < head->pair.first)
            head->left = insert(head->left, k, val);
        else if (k > head->pair.first)
            head->right = insert(head->right, k, val);
    ...
    Node* erase(Node* head, const T1 key)
    {
        if (!head)
            return nullptr;
